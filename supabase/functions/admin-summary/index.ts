...
Deno.serve(async (request) => {
  ...
  const [users, subscriptions, workspaces, openIncidents, pendingDeliveries, incidents, backups, reportRuns, aiRequests, exportJobs, healthSnapshots, restoreDrills, subjectRequests, rateLimitEvents, aiProviders, aiProviderUsage, aiProviderAttempts, stripeAcceptanceRuns, stripeAcceptanceEvidence, notificationAcceptanceRuns, notificationAcceptanceEvidence, notificationAttempts, rolloutStates, readinessRuns, readinessEvidence, loadTests, stabilityIncidents] = await Promise.all([
    ...
    rows('ai_request_logs', 'select=id,workspace_id,user_id,report_run_id,report_id,request_id,acceptance_run_id,entry_point,operation,task_type,agent_key,requested_provider,provider,provider_attempts,provider_config_fingerprints,fallback_used,retry_count,data_disclosure,status,model,input_tokens,output_tokens,total_tokens,estimated_cost_usd,duration_ms,http_status,error_code,data_version,search_enabled,created_at&order=created_at.desc&limit=500'),
    ...
    rows('ai_provider_attempt_logs', 'select=request_id,attempt_no,acceptance_run_id,workspace_id,task_type,agent_key,provider,model,config_fingerprint,status,http_status,total_tokens,estimated_cost_usd,error_code,duration_ms,created_at&order=created_at.desc&limit=500'),
