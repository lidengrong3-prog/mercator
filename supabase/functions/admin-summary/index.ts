...
Deno.serve(async (request) => {
  ...
  const [users, subscriptions, workspaces, openIncidents, pendingDeliveries, incidents, backups] = await Promise.all([
  ...
  return json({ role: adminRows[0].role, counts: { users, subscriptions, workspaces, open_incidents: openIncidents, pending_deliveries: pendingDeliveries }, incidents, backups, generated_at: new Date().toISOString() }, 200, origin);
});
