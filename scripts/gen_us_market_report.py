...
# -*- coding: utf-8 -*-
"""
gen_us_market_report.py — 美国市场「全品类」调研报告 (PDF) 生成器（参数化）

读取 data/us_market/<cat>.json（由 collect_us_market.py 产出），
对标《美国珠宝全渠道市场调研报告》的综合 PDF，按品类各生成一份。
...
运行：
  python scripts/gen_us_market_report.py                # 全部 8 品类
  python scripts/gen_us_market_report.py --category electronics   # 单品类
...
OUT_DIR = os.path.join(ROOT, "reports", "us_market")
...
def load(cat_key):
    with open(os.path.join(DATA_DIR, "%s.json" % cat_key), "r", encoding="utf-8") as f:
        return json.load(f)
...
def styles():
    ...
    s["title"] = ParagraphStyle("title", parent=ss["Title"], fontName=FONT,
                                fontSize=25, textColor=NAVY, leading=31, alignment=TA_CENTER)
    s["subtitle"] = ParagraphStyle("subtitle", parent=ss["Normal"], fontName=FONT,
                                   fontSize=12.5, textColor=SEA, alignment=TA_CENTER, spaceAfter=6)
    s["h1"] = ParagraphStyle("h1", parent=ss["Heading1"], fontName=FONT, fontSize=16,
                             textColor=NAVY, spaceBefore=14, spaceAfter=8)
    s["h2"] = ParagraphStyle("h2", parent=ss["Heading2"], fontName=FONT, fontSize=12.5,
...
def build(d, st, cat_name):
    ...
    # ---------- 封面 ----------
    flow.append(Spacer(1, 36 * mm))
    flow.append(P("%s · 美国市场深度调研报告" % cat_name, st["title"]))
    flow.append(Spacer(1, 6 * mm))
    flow.append(P("US %s Market Intelligence Report" % (c.get("name_en", "")), st["subtitle"]))
    flow.append(Spacer(1, 4 * mm))
    flow.append(P("数据自动更新版 · 单点市场全板块情报", st["subtitle"]))
    ...
    flow.append(P("说明：政策动态与预警中心由 Federal Register 官方 API 实时抓取，"
                  ...
                  "（Statista / eMarketer / US Census / USTR / 各监管机构），均标注来源与截止日期。", st["small"]))
    flow.append(PageBreak())

    # ---------- 执行摘要 ----------
    flow.append(P("执行摘要", st["h1"]))
    flow.append(P(c.get("opportunity", ""), st["body"]))
    flow.append(P("本报告围绕用户指定的 5 大板块——国家市场板块内容、电商平台档案、平台规则、"
                  "政策动态、预警中心——进行单点（美国·%s）全量情报采集，数据真实、来源可溯、"
                  "且纳入 4 小时自动更新流水线。" % cat_name, st["body"]))
    ...
    # ---------- 第一部分 宏观全景 ----------
    flow.append(P("第一部分　美国%s行业宏观全景" % cat_name, st["h1"]))
    ...
    for p in d.get("platforms", []):
        ...
        flow.append(Spacer(1, 2 * mm))
    flow.append(P("2.2 渠道横向对比总表", st["h2"]))
    ...
    flow.append(P("以下为各平台与美方监管机构对%s品类的真实合规红线，违规可致下架、罚款或撤销资质。" % cat_name, st["body"]))
    ...
    flow.append(P("本节由 Federal Register 官方 API 实时抓取，每次运行刷新；标记为 live 的条目为"
                  "自动采集，其余为权威来源引用的当下政策/预警。", st["body"]))
    ...
    for sec in ["country", "platforms", "rules", "policies", "alerts"]:
        sec_data = d.get(sec)
        items = sec_data if isinstance(sec_data, list) else [sec_data]
        for it in items:
    ...
    flow.append(P("生成工具：JAY观海 跨境市场情报系统 · scripts/collect_us_market.py + gen_us_market_report.py", st["small"]))

    return flow


def footer(canvas, doc, cat_name):
    canvas.saveState()
    canvas.setFont(FONT, 8)
    canvas.setFillColor(GREY)
    canvas.drawString(20 * mm, 12 * mm, "美国%s市场深度调研报告 · 数据自动更新版" % cat_name)
...
def main():
    ...
    p.add_argument("--category", help="仅生成指定品类 key（如 electronics）")
    args = p.parse_args()
    ...
    if args.category:
        ...
        keys = sorted(glob.glob(os.path.join(DATA_DIR, "*.json")))
        keys = [os.path.basename(k)[:-5] for k in keys if os.path.basename(k) not in ("index.json",)]
        # 仅保留真实品类（排除 index）
        real = {"electronics", "apparel", "home", "beauty", "toys", "sports", "auto", "supplements"}
        keys = [k for k in keys if k in real]

    st = styles()
    for k in keys:
        d = load(k)
        cat_name = d.get("country", {}).get("market", k)
        out = os.path.join(OUT_DIR, "%s_report.pdf" % k)
        doc = SimpleDocTemplate(out, pagesize=A4,
                                leftMargin=20 * mm, rightMargin=20 * mm,
                                topMargin=18 * mm, bottomMargin=18 * mm,
                                title="美国%s市场深度调研报告" % cat_name, author="JAY观海")
        doc.build(build(d, st, cat_name), onFirstPage=lambda c, doc_: footer(c, doc_, cat_name),
                  onLaterPages=lambda c, doc_: footer(c, doc_, cat_name))
