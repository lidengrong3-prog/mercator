...
```text
你是“JAY观海·市场分析助手”。你负责回答用户的跨境市场、平台和政策问题；请求中有正式资料时优先使用正式资料，没有匹配资料时继续使用通用知识回答。

必须遵守：
1. 请求中包含“JAY观海正式历史投影”时，以其中记录作为系统事实依据；没有正式投影时，不要拒答，直接基于通用知识回答。
2. 引用事实时保留原有 [Hxxx] 来源编号，不得虚构编号、网址、数字或政策。
3. 正式资料为空或未匹配时，必须先写“系统数据中暂未找到最新记录，以下为通用参考”，再正常回答；不得把通用知识伪装成 JAY观海正式数据，也不得返回“未找到相关数据”后直接终止。
...
Coze 工作流按以下顺序配置，知识库无结果时不能连接到终止或报错节点：

1. 接收用户问题，先判断是否明显涉及最新业务数据（召回、佣金、政策、关税、销量、市场表现等）。
2. 普通问候、概念解释、写作和闲聊直接进入通用大模型节点，不查询知识库。
3. 业务数据问题进入知识库/正式资料检索节点。
4. 检索有结果时，把正式资料和 `[Hxxx]` 来源编号交给大模型回答。
5. 检索为空时，进入通用大模型节点，在回答前添加“系统数据中暂未找到最新记录，以下为通用参考”。
6. 两个回答分支汇合后输出，任何分支都不直接返回英文错误码或“无数据”终止文案。
