...
- `.github/workflows/encrypted-backup.yml` 每日生成加密 PostgreSQL logical dump，并将 `private-raw-data`、`reports` Storage 对象打包加密后写入私有 bucket；Actions 只保留校验和对象路径摘要。
- `.github/workflows/restore-drill.yml` 每月第一天将最新数据库备份恢复到独立的 `RESTORE_DRILL_DB_URL`，执行用户、工作区、报告、导出和来源表 smoke 查询，并写入 `backup_restore_drills`。
- 必须配置 `SUPABASE_DB_URL`、`BACKUP_ENCRYPTION_KEY` 和隔离数据库 `RESTORE_DRILL_DB_URL`。隔离数据库不能使用生产连接字符串。
