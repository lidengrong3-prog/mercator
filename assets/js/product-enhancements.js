...
(function(){
  ...
  function jayOpenGlossary(){
    var rows=Object.keys(GLOSS_TERMS).map(function(k){return '<tr><td data-ui-style="padding:4px 10px;border-bottom:1px solid #eee;white-space:nowrap"><b>'+k+'</b></td><td data-ui-style="padding:4px 10px;border-bottom:1px solid #eee">'+GLOSS_TERMS[k]+'</td></tr>';}).join('');
  ...
  window.jayInitFeedback=function(){
    ...
    function render(){
      ...
      if(v){ box.innerHTML='<span class="jay-fb-q">本页（'+JAY_FB_CTX+'）已反馈 · '+(v.vote>0?'👍 有用':'👎 待改进')+'</span><button type="button" data-action="jayFbClear()">修改</button>'; box.classList.add('done'); }
      else { box.classList.remove('done'); box.innerHTML='<span class="jay-fb-q">本页内容对您有帮助吗？</span><button type="button" data-action="jayFbVote(1)">👍 有用</button><button type="button" data-action="jayFbVote(-1)">👎 待改进</button>'; }
    }
  ...
  window.jayInitRoleCard=function(){
    ...
    var html='<div class="jay-card" id="jay-role-card" data-ui-style="margin-bottom:16px"><div class="jay-section-title">用户角色</div>'
      +'<div class="jay-muted" data-ui-style="margin-bottom:10px">选择你的角色，系统会优先呈现与你最相关的功能入口。</div>'
      +'<div class="jay-role-bar"><div class="jay-seg" id="jay-role-seg">'
      +Object.keys(labels).map(function(k){return '<button type="button" data-role="'+k+'" class="'+(k===role?'active':'')+'" data-action="jaySetRole(\''+k+'\')">'+labels[k]+'</button>';}).join('')
      +'</div></div></div>';
...
function jayRenderBell(){
  ...
  var head=document.createElement('div');head.className='bp-head';
  var title=document.createElement('span');title.textContent=items.length?'通知中心（'+items.length+'）':'通知中心';head.appendChild(title);
  if(items.length){
    var all=document.createElement('button');all.type='button';all.className='help-entry';all.textContent='查看全部';
    all.addEventListener('click',function(){switchPage('alerts');panel.classList.remove('show');});head.appendChild(all);
  }
  panel.appendChild(head);
  if(items.length===0){var empty=document.createElement('div');empty.className='bp-empty';empty.textContent='暂无新通知';panel.appendChild(empty);return;}
  var allowedPages={alerts:true,policies:true,rules:true};
  items.forEach(function(it){
    var item=document.createElement('button');item.type='button';item.className='bp-item'+(it.read?' read':'');
    var dot=document.createElement('span');dot.className='bp-dot';dot.style.backgroundColor=it.color;
    item.appendChild(dot);item.appendChild(document.createTextNode(String(it.text||'')));
    item.addEventListener('click',function(){
      var page=allowedPages[it.page]?it.page:'alerts';
      if(it.notification)jayOpenNotification(String(it.id||''),page);else switchPage(page);
...
function jayBuildSettingsPage(){
  ...
  section.innerHTML='<div class="st-wrap">'
    ...
    +'<button type="button" class="st-side-btn active" data-st-tab="account" data-action="stSwitchTab(\'account\')"><i data-lucide="user-round"></i>个人账号</button>'
    +'<button type="button" class="st-side-btn" data-st-tab="alerts" data-action="stSwitchTab(\'alerts\')"><i data-lucide="bell-ring"></i>预警订阅</button>'
    +'<button type="button" class="st-side-btn" data-st-tab="prefs" data-action="stSwitchTab(\'prefs\')"><i data-lucide="settings-2"></i>工作区偏好</button>'
    +'<button type="button" class="st-side-btn" data-st-tab="team" data-action="stSwitchTab(\'team\')"><i data-lucide="users-round"></i>团队与权限</button>'
    +'<button type="button" class="st-side-btn" data-st-tab="system" data-action="stSwitchTab(\'system\')"><i data-lucide="activity"></i>系统状态</button>'
    +'</nav><div class="st-main">'
    +'<section class="st-tab active" id="st-tab-account"><h2 data-ui-style="margin:0 0 4px;font-size:20px">个人账号</h2><p class="st-section-desc">Supabase Auth 登录身份与个人资料</p>'
    +'<div class="st-section"><div class="st-avatar-upload"><div class="st-avatar-big" id="st-avatar-big">U</div><div><div id="st-tier-badge" data-ui-style="font:11px DM Mono;color:#3b7ab8;background:rgba(59,125,216,.12);padding:2px 8px;border-radius:10px;display:inline-block">会员：免费版</div><p data-ui-style="font-size:11px;color:var(--muted);margin:8px 0 0">资料按登录账号同步</p></div></div>'
    ...
    +'<button type="button" class="st-btn st-btn-primary" id="st-account-save" data-action="stSaveAccount()">保存修改</button></div>'
    +'<hr class="st-divider"><div class="st-section"><h3 class="st-section-title">账号安全</h3><div class="st-api-row"><label>认证服务</label><div class="st-service-status"><span class="health-dot"></span><b>Supabase Auth</b><small>当前浏览器会话</small></div><button type="button" class="st-btn st-btn-outline st-btn-sm" data-action="stSendPasswordReset()">发送密码重置邮件</button></div></div>'
    +'<hr class="st-divider"><div class="st-section"><h3 class="st-section-title">数据权利</h3><p class="st-section-desc">你可以导出自己的账号数据，或申请删除账号及相关文件。删除前请先完成工作区所有权转移。</p><div class="st-inline-actions"><button type="button" class="st-btn st-btn-outline st-btn-sm" data-action="jayRequestDataExport()">导出我的数据</button><button type="button" class="st-btn st-btn-outline st-btn-sm" data-action="jayRequestAccountDeletion()">申请删除账号</button><span class="st-muted-inline" id="st-data-request-status"></span></div></div>'
    ...
    +'<section class="st-tab" id="st-tab-alerts"><h2 data-ui-style="margin:0 0 4px;font-size:20px">预警订阅</h2><p class="st-section-desc">订阅类型和当前市场范围随账号跨设备同步</p><div class="st-section"><h3 class="st-section-title">关注事件</h3><div class="st-alert-checks">'
    +'<label class="st-alert-check"><input type="checkbox" data-sub="product" checked> 商品价格、销量或评分异动</label><label class="st-alert-check"><input type="checkbox" data-sub="shop" checked> 店铺经营指标异常</label><label class="st-alert-check"><input type="checkbox" data-sub="content" checked> 新品与热门内容</label><label class="st-alert-check"><input type="checkbox" data-sub="policy" checked> 政策与合规风险</label><label class="st-alert-check"><input type="checkbox" data-sub="competition"> 类目竞争加剧</label><label class="st-alert-check"><input type="checkbox" data-sub="fx"> 汇率大幅波动</label></div><button type="button" class="st-btn st-btn-primary st-btn-sm" data-action="stSaveSubPref()" data-ui-style="margin-top:12px">保存订阅</button></div>'
    +'<hr class="st-divider"><div class="st-section"><h3 class="st-section-title">通知渠道</h3><div id="st-notification-service-note" class="st-inline-notice">正在读取服务端渠道状态…</div><div class="st-api-grid"><div class="st-api-row"><label>站内通知</label><div class="st-service-status"><span class="health-dot"></span><b>已启用</b><small>预警中心与顶部消息</small></div><button type="button" class="st-btn st-btn-outline st-btn-sm" data-action="stSendTestPush()">发送站内测试</button></div></div>'
    +'<div class="st-notification-channels">'
    +'<article class="st-notification-channel" data-notification-channel="email"><div class="st-notification-channel-head"><div><b>邮件通知</b><small id="st-channel-email-target">登录邮箱</small></div><label class="st-channel-switch"><input id="st-channel-email-enabled" type="checkbox">启用</label></div><div class="st-channel-state"><span class="health-dot" id="st-channel-email-dot"></span><span id="st-channel-email-state">读取中</span></div><div class="st-channel-actions"><button type="button" class="st-btn st-btn-primary st-btn-sm" id="st-channel-email-save" data-action="stSaveNotificationChannel(\'email\')">保存</button><button type="button" class="st-btn st-btn-outline st-btn-sm" id="st-channel-email-test" data-action="stTestNotificationChannel(\'email\')">发送测试</button></div></article>'
    +'<article class="st-notification-channel" data-notification-channel="wecom"><div class="st-notification-channel-head"><div><b>企业微信</b><small id="st-channel-wecom-target">机器人 Webhook</small></div><label class="st-channel-switch"><input id="st-channel-wecom-enabled" type="checkbox">启用</label></div><div class="st-channel-state"><span class="health-dot" id="st-channel-wecom-dot"></span><span id="st-channel-wecom-state">读取中</span></div><input class="st-input st-channel-webhook" id="st-channel-wecom-url" type="password" autocomplete="new-password" placeholder="企业微信机器人 Webhook"><div class="st-channel-actions"><button type="button" class="st-btn st-btn-primary st-btn-sm" id="st-channel-wecom-save" data-action="stSaveNotificationChannel(\'wecom\')">保存</button><button type="button" class="st-btn st-btn-outline st-btn-sm" id="st-channel-wecom-test" data-action="stTestNotificationChannel(\'wecom\')">发送测试</button></div></article>'
    +'<article class="st-notification-channel" data-notification-channel="feishu"><div class="st-notification-channel-head"><div><b>飞书</b><small id="st-channel-feishu-target">机器人 Webhook</small></div><label class="st-channel-switch"><input id="st-channel-feishu-enabled" type="checkbox">启用</label></div><div class="st-channel-state"><span class="health-dot" id="st-channel-feishu-dot"></span><span id="st-channel-feishu-state">读取中</span></div><input class="st-input st-channel-webhook" id="st-channel-feishu-url" type="password" autocomplete="new-password" placeholder="飞书机器人 Webhook"><div class="st-channel-actions"><button type="button" class="st-btn st-btn-primary st-btn-sm" id="st-channel-feishu-save" data-action="stSaveNotificationChannel(\'feishu\')">保存</button><button type="button" class="st-btn st-btn-outline st-btn-sm" id="st-channel-feishu-test" data-action="stTestNotificationChannel(\'feishu\')">发送测试</button></div></article>'
    +'</div></div></section>'
    +'<section class="st-tab" id="st-tab-prefs"><h2 data-ui-style="margin:0 0 4px;font-size:20px">工作区偏好</h2><p class="st-section-desc">偏好设置随账号同步</p><div id="st-role-host"></div>'
    +'<div class="st-section"><h3 class="st-section-title">显示单位</h3><div class="st-form-row"><div class="st-form-group"><label>货币</label><div data-ui-style="display:flex;gap:8px"><button type="button" id="st-currency-cny" class="st-btn st-btn-sm st-btn-primary" data-action="stCurrency(\'cny\')">人民币</button><button type="button" id="st-currency-usd" class="st-btn st-btn-sm st-btn-outline" data-action="stCurrency(\'usd\')">美元</button></div></div><div class="st-form-group"><label>数值单位</label><div data-ui-style="display:flex;gap:8px"><button type="button" id="st-unit-wan" class="st-btn st-btn-sm st-btn-primary" data-action="stUnit(\'wan\')">万</button><button type="button" id="st-unit-m" class="st-btn st-btn-sm st-btn-outline" data-action="stUnit(\'m\')">百万</button></div></div></div>'
    +'</div></section>'
    +'<section class="st-tab" id="st-tab-team"><h2 data-ui-style="margin:0 0 4px;font-size:20px">团队与权限</h2><p class="st-section-desc">工作区成员共享权限边界；数据仍按登录身份和工作区策略隔离</p>'
    +'<div id="st-workspace-unavailable" class="st-inline-notice" data-ui-style="display:none"></div>'
    +'<div id="st-workspace-content">'
    +'<div class="st-section st-workspace-card"><div class="st-section-head"><div><h3 class="st-section-title">当前工作区</h3><p class="st-section-desc" data-ui-style="margin-bottom:0">工作区所有者可修改名称并管理成员</p></div><span class="st-role-pill" id="st-workspace-role">读取中</span></div>'
    +'<div class="st-form-row" data-ui-style="margin-top:16px"><div class="st-form-group"><label for="st-workspace-select">切换工作区</label><select class="st-select" id="st-workspace-select" data-change-action="stSwitchWorkspace(this.value)"><option value="">正在读取工作区…</option></select></div><div class="st-form-group"><label for="st-workspace-name">工作区名称</label><input class="st-input" id="st-workspace-name" maxlength="80" autocomplete="organization"></div></div>'
  +'<div class="st-form-row"><div class="st-form-group"><label>席位使用</label><input class="st-input" id="st-workspace-member-count" readonly value="读取中"></div><div class="st-form-group"><label>当前工作区权益</label><input class="st-input" id="st-workspace-billing-meta" readonly value="读取中"></div></div>'
    +'<div class="st-inline-actions"><button type="button" class="st-btn st-btn-primary st-btn-sm" id="st-workspace-save" data-action="stSaveWorkspaceName()">保存工作区</button><button type="button" class="st-btn st-btn-outline st-btn-sm" data-action="jayRequestWorkspaceDeletion()">申请删除工作区</button><span class="st-muted-inline" id="st-workspace-meta">读取工作区信息…</span></div></div>'
    +'<hr class="st-divider"><div class="st-section"><div class="st-section-head"><div><h3 class="st-section-title">成员</h3><p class="st-section-desc" data-ui-style="margin-bottom:0">只有所有者和管理员可以调整成员权限</p></div><span class="st-muted-inline" id="st-workspace-permission-hint"></span></div>'
  +'<div class="st-table-wrap"><table class="st-members-table"><thead><tr><th>成员</th><th>角色</th><th>状态</th><th>AI Token</th><th>报告</th><th>导出</th><th>加入时间</th></tr></thead><tbody id="st-workspace-members"><tr><td colspan="7" class="st-table-empty">正在读取成员…</td></tr></tbody></table></div></div>'
    +'<hr class="st-divider"><div class="st-section" id="st-workspace-invite-section"><h3 class="st-section-title">邀请成员</h3><p class="st-section-desc">系统会向对方发送 7 天有效的邀请邮件。发送前必须明确选择角色，只有邮件服务确认接收后才显示“已发送”。</p><div class="st-form-row"><div class="st-form-group"><label for="st-invite-email">邮箱</label><input class="st-input" id="st-invite-email" type="email" placeholder="name@company.com" autocomplete="email"></div><div class="st-form-group"><label for="st-invite-role">角色</label><select class="st-select" id="st-invite-role"><option value="">请选择角色</option><option value="viewer">查看者</option><option value="editor">编辑者</option><option value="admin">管理员</option></select></div></div><div class="st-inline-actions"><button type="button" class="st-btn st-btn-primary st-btn-sm" id="st-invite-submit" data-action="stCreateInvite()">发送邀请</button><span class="st-muted-inline">邀请有效期 7 天</span></div></div>'
    +'<div class="st-section" id="st-workspace-invites-section"><h3 class="st-section-title">邀请记录</h3><div class="st-table-wrap"><table class="st-members-table"><thead><tr><th>邮箱</th><th>角色</th><th>邀请状态</th><th>邮件投递</th><th>有效期</th><th>操作</th></tr></thead><tbody id="st-workspace-invites"><tr><td colspan="6" class="st-table-empty">暂无邀请记录</td></tr></tbody></table></div></div>'
    +'</div></section>'
    +'<section class="st-tab" id="st-tab-system"><h2 data-ui-style="margin:0 0 4px;font-size:20px">系统状态</h2><p class="st-section-desc">基于当前会话和最近一次数据质量报告</p><div class="st-section"><div class="st-status-grid"><article class="st-status-item"><div class="st-status-label"><i data-lucide="database"></i>用户数据</div><div class="st-service-status"><span class="health-dot" id="st-system-auth-dot"></span><b>Supabase</b><small id="st-system-auth">读取中</small></div></article><article class="st-status-item"><div class="st-status-label"><i data-lucide="badge-check"></i>数据发布</div><div class="st-service-status"><span class="health-dot" id="st-system-quality-dot"></span><b id="st-system-quality">读取中</b><small id="st-system-updated">读取中</small></div></article><article class="st-status-item"><div class="st-status-label"><i data-lucide="refresh-cw"></i>自动采集</div><div class="st-service-status"><span class="health-dot" data-ui-style="background:#94a3b8"></span><b>GitHub Actions</b><small>计划每 4 小时执行，以工作流结果为准</small></div></article><article class="st-status-item"><div class="st-status-label"><i data-lucide="sparkles"></i>AI 服务</div><div class="st-service-status"><span class="health-dot" data-ui-style="background:#94a3b8"></span><b>Edge Function</b><small>登录调用成功后才能确认可用</small></div></article></div><div class="st-status-actions"><button type="button" class="st-btn st-btn-outline" data-action="switchPage(\'data\')">查看数据质量明细</button></div></div></section>'
    +'</div></div>';
}
...
/* ===== 数据权利：导出与删除请求 ===== */
async function jayRequestDataExport(){
  var status=document.getElementById('st-data-request-status');
  if(status)status.textContent='正在准备导出…';
  try{
    var result=await jayFunctionRequest('data-subject-request',{request_type:'export'},{timeout:60000,retryOnNetwork:true,requestId:'data-export-'+Date.now()});
    var blob=new Blob([JSON.stringify(result.export||result,null,2)],{type:'application/json;charset=utf-8'});
    var link=document.createElement('a');link.href=URL.createObjectURL(blob);link.download='jay-guanhai-data-export.json';link.click();setTimeout(function(){URL.revokeObjectURL(link.href);},1000);
