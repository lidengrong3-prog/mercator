(function(){
  ...
  function card(item){
    ...
    var learning=state.tab==='academy'?'<div class="resource-progress"><div class="resource-progress-head"><span>学习进度</span><b>'+progressPercent+'%</b></div><div class="resource-progress-track"><i data-ui-style="width:'+progressPercent+'%"></i></div><button type="button" class="resource-card-action resource-progress-toggle" data-progress="'+(progressPercent>=100?'reset':'complete')+'">'+(progressPercent>=100?'重新学习':'标记完成')+'</button></div>':'';
  ...
  function academyOutline(data){
    ...
    var moduleHtml=modules.map(function(module,moduleIndex){
      ...
      var lessonHtml=lessons.map(function(lesson,lessonIndex){
        ...
        return '<article class="academy-lesson" data-lesson-id="'+esc(lesson.id)+'"><div class="academy-lesson-main"><span class="academy-lesson-type">'+esc(lesson.lesson_type==='video'?'视频':(lesson.lesson_type==='article'?'文章':'资料'))+'</span><div><strong>'+esc(lesson.title)+'</strong><p>'+esc(lesson.summary||'')+'</p></div></div>'+resourceButtons+'<div class="academy-lesson-meta"><span>'+esc(academyDuration(lesson.duration_seconds))+'</span><span>'+percent+'%</span><div class="academy-lesson-progress-track"><i data-ui-style="width:'+percent+'%"></i></div><button type="button" class="st-btn st-btn-outline academy-lesson-progress" data-lesson-id="'+esc(lesson.id)+'" data-progress-action="'+(percent>=100?'reset':'complete')+'">'+action+'</button>'+(lesson.lesson_type==='video'?'<button type="button" class="st-btn st-btn-outline academy-lesson-play" data-lesson-id="'+esc(lesson.id)+'"><i data-lucide="play"></i>播放</button>':'')+videoUpload+materialUpload+lessonOrder+'</div></article>';
      }).join('');
