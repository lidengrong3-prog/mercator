...
function prApplyFilters(){
    try{products.forEach(function(p){if(Array.isArray(p)&&typeof p[6]==='string'&&p[6].indexOf('$')===0&&p[6].indexOf('RMB')<0){var m=p[6].match(/\$(\d+(?:\.\d+)?)\s*-\s*(\d+(?:\.\d+)?)/);if(m){p[7]=Math.round(parseFloat(m[1])*7.2)+'-'+Math.round(parseFloat(m[2])*7.2);}else{var s2=p[6].match(/\$(\d+(?:\.\d+)?)/);if(s2){p[7]=Math.round(parseFloat(s2[1])*7.2);}}}});}catch(e){}
...
function prRenderTable(list){
  ...
  $('#pr-count').textContent='● '+list.length+' / '+products.length+' 条实时数据';
  prUpdateBatchBar();
}
...
function prExportPDF(){
  ...
  if(jayPrintMarkdownReport('产品全域雷达 - 竞品分析报告',md,'商品成交额为系统估算值，请结合平台后台与原始来源复核。')) toast('已打开打印页，可另存为 PDF');
}
...
// ========== G1 竞品深化：品类结构 / 流量结构 / 竞品对标 ==========
function shCatStructure(s){
  var main=s[6];
  var pool=['美妆个护','3C数码','家居日用','时尚服饰','食品饮料','母婴用品','户外运动','汽车配件','饰品配件','保健品','宠物用品','家居家装','家居厨房','日用百货'];
  var g=Math.abs(parseFloat(s[4])||0);
  var top1=Math.round(40+Math.min(28, g/4));
  var list=[[main, top1]];
  var remaining=100-top1;
  var others=pool.filter(function(c){return c!==main;});
  for(var i=0;i<3&&remaining>0;i++){
    var pct=Math.min(remaining, Math.round(Math.random()*12)+6);
    list.push([others[(i*3+1)%others.length], pct]);
    remaining-=pct;
  }
  if(remaining>0) list.push(['其他', remaining]);
  return list;
}
function shHHI(cats){ var h=0; cats.forEach(function(c){h+=c[1]*c[1];}); return Math.round(h/100); }
function shAOV(s){
  var map={'美妆个护':[28,55],'3C数码':[45,120],'家居家装':[35,85],'家居日用':[18,55],'家居厨房':[25,70],'家居家电':[40,95],'时尚服饰':[20,58],'食品饮料':[12,38],'母婴用品':[25,65],'户外运动':[38,95],'汽车配件':[35,110],'饰品配件':[15,48],'保健品':[28,85],'宠物用品':[18,55],'日用百货':[12,42]};
  var r=map[s[6]]||[22,60];
  return Math.round(r[0]+Math.random()*(r[1]-r[0]));
}
function shTrafficStructure(s){
  var content=['TikTok Shop','Instagram Shop / Facebook Shop','YouTube Shopping','Pinterest Shop','SHEIN'];
  var isContent=content.indexOf(s[1])>=0;
  var t = isContent ? [['直播',28],['达人合作',30],['自然搜索',22],['推荐流量',13],['付费广告',7]] : [['自然搜索',40],['推荐流量',24],['付费广告',16],['达人合作',11],['直播',9]];
  t.forEach(function(x){ x[1]=Math.max(4, x[1]+Math.round(Math.random()*6-3)); });
  var tot=t.reduce(function(a,b){return a+b[1];},0);
  t.forEach(function(x){ x[1]=Math.round(x[1]/tot*100); });
  return t;
}
function shRing(seg){
  var acc=0, stops=[], cols=['#2c5f8a','#4a90d9','#7fb3e0','#c8a84e','#9bbf8a'];
  seg.forEach(function(x,i){ var st=acc; acc+=x[1]; stops.push(cols[i%5]+' '+st+'% '+acc+'%'); });
  return 'conic-gradient('+stops.join(',')+')';
}
function shBenchmarkFor(s, idx){
  var plat=s[1], cat=s[6], market=s[2];
  function grp(f){ return shops.filter(f); }
  var peers=grp(function(x){return x[1]===plat&&x[6]===cat;});
  var scope='同平台·同品类';
  if(peers.length<2){ peers=grp(function(x){return x[2]===market&&x[6]===cat;}); scope='同市场·同品类'; }
  if(peers.length<2){ peers=grp(function(x){return x[6]===cat;}); scope='全平台·同品类'; }
  if(peers.length<2){ return {peers:[],scope:'',avg:null,top:null,gapText:''}; }
  var others=peers.filter(function(x){ return shops.indexOf(x)!==idx; });
  if(others.length<1) others=peers;
  var sum={gmv:0,growth:0,sku:0,fans:0,score:0,wave:0,aov:0};
  others.forEach(function(x){
    sum.gmv+=shParseGMV(x[3]); sum.growth+=parseFloat(x[4])||0; sum.sku+=parseInt(x[7])||0;
    sum.fans+=parseInt(x[10])||0; sum.score+=parseFloat(x[11])||0; sum.wave+=parseFloat(x[8])||0; sum.aov+=shAOV(x);
  });
  var n=others.length;
  var avg={gmv:sum.gmv/n,growth:sum.growth/n,sku:sum.sku/n,fans:sum.fans/n,score:sum.score/n,wave:sum.wave/n,aov:sum.aov/n};
  var top=others.slice().sort(function(a,b){return shParseGMV(b[3])-shParseGMV(a[3]);})[0];
  var myGmv=shParseGMV(s[3]);
  var mult = avg.gmv>0 ? (myGmv/avg.gmv) : 1;
  var gap = mult>=1.3 ? '你的规模已领先同类均值 '+mult.toFixed(1)+' 倍，重点在扩品类与守住标杆位。'
            : mult>=0.9 ? '与同类均值基本持平，增速若高于均值则有望弯道超车。'
            : '规模约为同类均值的 '+mult.toFixed(2)+' 倍，建议对标其品类结构与流量打法补齐差距。';
  return {peers:others,scope:scope,avg:avg,top:{name:top[0],gmv:top[3]},gapText:gap};
}
function shGenCompetitorReport(name,cat,plat){
  var prompt='请生成「'+name+'」竞品对标分析报告：主营'+cat+'、平台'+plat+'。拆解其品类结构与流量来源结构，与同类头部标杆对比差距，输出差异化竞争建议与供应链策略。';
  shCloseModal();
  switchPage('report');
  setTimeout(function(){
    var ta=document.getElementById('rp-v2-custom-prompt');
    if(ta){ ta.value=prompt; ta.focus(); }
    var card=document.querySelector('.rp-v2-tpl-card[data-tpl="competitor-analysis"]');
...
function shRenderCompareTab(){
  ...
  shops.forEach(function(x){cats[x[6]]=1;markets[x[2]]=1;plats[x[1]]=1;});
  ...
  html+='<p style="margin:0 0 10px;font-size:13px;color:var(--ink)"><b>竞品对标</b><span style="color:var(--muted);font-weight:400"> · 选品类即可看同类标杆长什么样（演示数据）</span></p>';
  ...
  el.innerHTML=html;
  var dc=document.getElementById('sh-cmp-cat'); if(dc){dc.value='美妆个护';}
  shRunCompare();
}
function shRunCompare(){
  ...
  if(!peers.length){ res.innerHTML='<p style="color:var(--muted);font-size:13px">该条件下暂无示例店铺，请放宽筛选。</p>'; return; }
  peers.sort(function(a,b){return shParseGMV(b[3])-shParseGMV(a[3]);});
  var top5=peers.slice(0,5);
  var n=peers.length;
  var avgGrow=peers.reduce(function(a,x){return a+(parseFloat(x[4])||0);},0)/n;
  var avgAov=peers.reduce(function(a,x){return a+shAOV(x);},0)/n;
  var html='';
  html+='<div style="font-size:12px;color:var(--muted);margin-bottom:10px">匹配 <b style="color:var(--ink)">'+n+'</b> 家店铺 · 平均增速 <b style="color:var(--ink)">+'+(avgGrow).toFixed(1)+'%</b> · 平均客单价(演示) <b style="color:var(--ink)">$'+avgAov.toFixed(0)+'</b></div>';
  ...
  top5.forEach(function(x){
    ...
    html+='<div style="display:flex;justify-content:space-between;align-items:start;margin-bottom:6px"><strong style="font-size:14px;color:var(--ink)">'+x[0]+'</strong><span style="font-size:11px;color:var(--muted)">'+x[1]+'</span></div>';
    html+='<div style="font-size:12px;color:#555;line-height:1.7">月GMV <b>'+x[3]+'</b> · 增速 <b style="color:var(--green)">'+x[4]+'</b><br>主营 '+x[6]+' · 粉丝 '+((parseInt(x[10])||0)/10000).toFixed(1)+'万 · 评分 '+x[11]+'</div>';
    html+='<div style="display:flex;gap:8px;margin-top:10px">';
    html+='<button onclick="shShowDetail('+bi+')" style="font-size:11px;padding:4px 10px;border:1px solid var(--sea-deep);color:var(--sea-deep);border-radius:4px;background:transparent;cursor:pointer">🔍 拆解看板</button>';
    html+='<button onclick="shGenCompetitorReport(\''+escInline(x[0])+'\',\''+escInline(x[6])+'\',\''+escInline(x[1])+'\')" style="font-size:11px;padding:4px 10px;border:1px solid var(--orange);color:var(--orange);border-radius:4px;background:transparent;cursor:pointer">📊 对标报告</button>';
    html+='</div></div>';
  });
  html+='</div>';
  res.innerHTML=html;
...
function shInitFilters() {
  ...
  document.getElementById('sh-f-keyword').addEventListener('input', jayDeb('shApplyFilters'));
}
...
function shShowDetail(idx) {
  ...
  var growth = parseFloat(s[4]) || 0;
  var fans = parseInt(s[10]) || 0;
  var sku = parseInt(s[7]) || 0;
  var score = parseFloat(s[11]) || 0;
  var wave = parseFloat(s[8]) || 0;
  var trendData = [];
  for(var i=0; i<30; i++) {
  ...
  // 品类结构 / 流量结构 / 竞品对标（G1 演示派生）
  var mainCat = s[6];
  var cats = shCatStructure(s);
  var catTop1 = cats[0][1];
  var catHHI = shHHI(cats);
  var traffic = shTrafficStructure(s);
  var bench = shBenchmarkFor(s, idx);

  var catColors = ['#2c5f8a','#4a90d9','#7fb3e0','#c8a84e','#9bbf8a','#bbb'];

  var html = '';
  html += '<div style="font-size:11px;color:var(--muted);background:var(--sea-soft);border:1px solid var(--wave);border-radius:6px;padding:6px 10px;margin-bottom:12px">⚠️ 演示拆解：以下品类结构 / 流量结构 / 竞品对标为基于示例店铺的结构化示意，接入真实数据源后自动替换为实测拆解。</div>';
  // 4-block layout
  ...
  // Block 2: 品类结构拆解
  html += '<div style="border:1px solid #ddd;border-radius:8px;padding:14px">';
  html += '<h4 style="margin:0 0 6px;font-size:13px;color:var(--ink)">🏷️ 品类结构拆解</h4>';
  html += '<div style="font-size:11px;color:var(--muted);margin-bottom:8px">主营集中度 TOP1 <b style="color:var(--ink)">'+catTop1+'%</b> · 品类HHI <b style="color:var(--ink)">'+catHHI+'</b><span style="margin-left:4px">（越高越依赖单一爆品）</span></div>';
  cats.forEach(function(c, i) {
    ...
    html += '<div style="flex:0 0 90px;height:8px;background:#eee;border-radius:4px;overflow:hidden"><div style="height:100%;width:'+c[1]+'%;background:'+catColors[i]+'"></div></div>';
    html += '<span style="font-size:12px;font-weight:600;width:34px;text-align:right">'+c[1]+'%</span>';
    html += '</div>';
  });
  html += '</div>';
  ...
  // Block 2b: 流量结构拆解
  html += '<div style="border:1px solid #ddd;border-radius:8px;padding:14px">';
  html += '<h4 style="margin:0 0 10px;font-size:13px;color:var(--ink)">🌊 流量结构拆解</h4>';
  html += '<div style="display:flex;gap:14px;align-items:center">';
  html += '<div style="width:110px;height:110px;border-radius:50%;flex-shrink:0;background:'+shRing(traffic)+';position:relative"><div style="position:absolute;inset:26px;background:var(--paper);border-radius:50%"></div></div>';
  html += '<div style="flex:1">';
  traffic.forEach(function(t,i){
    html += '<div style="display:flex;align-items:center;gap:6px;font-size:11px;margin-bottom:4px"><div style="width:8px;height:8px;border-radius:2px;background:'+['#2c5f8a','#4a90d9','#7fb3e0','#c8a84e','#9bbf8a'][i%5]+'"></div><span style="flex:1">'+t[0]+'</span><b>'+t[1]+'%</b></div>';
  });
  html += '</div></div></div>';

  // Block 2c: 竞品对标
  html += '<div style="border:1px solid #ddd;border-radius:8px;padding:14px;grid-column:1 / -1">';
  html += '<h4 style="margin:0 0 4px;font-size:13px;color:var(--ink)">🎯 竞品对标 <span style="font-size:11px;color:var(--muted);font-weight:400">（'+bench.scope+' 同类均值）</span></h4>';
  if(bench.peers.length){
    var dims = [
      {k:'月GMV', you:baseGMV, avg:bench.avg.gmv, fmt:function(v){return '$'+(v/100).toFixed(0)+'万';}},
      {k:'增速', you:growth, avg:bench.avg.growth, fmt:function(v){return (v>=0?'+':'')+v.toFixed(1)+'%';}},
      {k:'客单价', you:shAOV(s), avg:bench.avg.aov, fmt:function(v){return '$'+v.toFixed(0);}},
      {k:'在售SKU', you:sku, avg:bench.avg.sku, fmt:function(v){return Math.round(v);}},
      {k:'粉丝数', you:fans, avg:bench.avg.fans, fmt:function(v){return (v/10000).toFixed(1)+'万';}},
      {k:'店铺评分', you:score, avg:bench.avg.score, fmt:function(v){return v.toFixed(1);}}
    ];
    dims.forEach(function(d){
      var maxv=Math.max(d.you,d.avg,1);
      html += '<div style="margin-bottom:8px">';
      html += '<div style="display:flex;justify-content:space-between;font-size:11px;margin-bottom:3px"><span>'+d.k+'</span><span><b style="color:var(--sea-deep)">你 '+d.fmt(d.you)+'</b> · 均值 '+d.fmt(d.avg)+'</span></div>';
      html += '<div style="display:flex;gap:4px;height:9px">';
      html += '<div style="flex:'+(d.you/maxv*100)+'%;background:var(--sea-deep);border-radius:3px 0 0 3px"></div>';
      html += '<div style="flex:'+(d.avg/maxv*100)+'%;background:#cfd8e3;border-radius:0 3px 3px 0"></div>';
      html += '</div></div>';
    });
    html += '<div style="font-size:11px;color:var(--muted);margin-top:6px;padding-top:6px;border-top:1px dashed #e3e9f0">头部标杆 <b style="color:var(--ink)">'+bench.top.name+'</b>（'+bench.top.gmv+'）：'+bench.gapText+'</div>';
  } else {
    html += '<p style="font-size:12px;color:var(--muted);margin:6px 0">示例数据中暂无同类可比店铺，接入真实数据源后将自动生成对标。</p>';
  }
  html += '</div>';
  ...
  // Bottom actions
  html += '<div style="display:flex;flex-wrap:wrap;gap:8px;margin-top:16px;padding-top:12px;border-top:1px solid #eee">';
  ...
  html += '<button onclick="shGenCompetitorReport(\'' + escInline(s[0]) + '\',\'' + escInline(s[6]) + '\',\'' + escInline(s[1]) + '\')" style="padding:6px 14px;border:1px solid var(--sea-deep);color:var(--sea-deep);border-radius:6px;background:transparent;cursor:pointer;font-size:12px">📊 生成竞品对标报告</button>';
  html += '</div>';
...
function shExportPDF() {
  ...
  if(jayPrintMarkdownReport('店铺追踪竞品分析报告',md.replace(/\\n/g, '\n'),'店铺经营数据为监测和估算结果，请以平台后台数据为准。')) toast('已打开打印页，可另存为 PDF');
}
...
async function jayLoadShopsFromCloud() {
  try {
    ...
    res.data.forEach(function(r) {
      ...
      if (local[id]) return;
      shops.push([r.shop_name, r.platform, r.market, r.gmv || 'US$ 0万', r.growth || '+0%', r.status || '正常', r.category || '未分类', 0, '+0%', r.tags || '', '0', '0', '云端']);
      local[id] = 1; added++;
    });
